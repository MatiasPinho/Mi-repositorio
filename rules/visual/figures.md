...
## Core rule
Use a visual when it makes a spatial, structural, causal, temporal or relational concept easier to understand than prose alone. This is especially important in subjects such as computer architecture, operating systems, networking, databases, electronics, algorithms and mathematics. A simple recognizable physical/conceptual likeness may also be useful when recognition itself improves understanding.
...
is never an automatic pencil effect applied to arbitrary pixels. Generated
illustrations follow the same truth boundary and are supporting likenesses,
never authoritative academic evidence.
...
- generated illustrations that carry exact academic relationships, labels, quantities, chronology or topology.
...
- `reinterpret`: the visual is a pedagogical derivation whose meaning can be
  ...
  unambiguous. A supporting generated illustration may also use `reinterpret`
  only under the strict illustration boundary below.
...
Do not force a fixed number of figures. `visual_not_needed` is a pedagogical decision, never a shortcut for saving tokens, image calls, review work or runtime.
...
## Visual medium for derived reinterpretations
A derived `reinterpret` also chooses the smallest medium that teaches the concept correctly.

### `visual_medium: diagram`
Use the established deterministic notebook-sketch path whenever the visual itself carries academic structure: flows, sequences, cycles, state transitions, timelines, hierarchies, concept/relationship maps, architecture/subsystem relationships, exact labels, order, direction, topology or grouping.

The planner owns semantic content and provenance; deterministic code owns final SVG geometry and notebook style. The model must not write raw SVG, pixel coordinates, colors or fonts. Missing `visual_medium` remains backward-compatible with `diagram` for existing plans/specs.

### `visual_medium: illustration`
Use a generated illustration only for a recognizable physical/conceptual likeness where exact topology is not the teaching payload—for example a generic CPU package, RAM module, disk, monitor, keyboard or peripheral.

Illustrations are intentionally supporting only:
- `need` must be `visual_helpful`, never `visual_required`;
- only simple visible cues explicitly supported by canonical knowledge may be requested;
- exact quantities, sequence, wiring, topology, formulas, dates, taxonomies and assessment-critical relationships stay out of generated pixels;
- do not request text, letters, numbers, labels or arrows inside the image;
- authoritative labels/explanations stay in prose/HTML or a deterministic diagram;
- never generate the whole study page as an image.

The planner supplies only the compact semantic illustration object defined in `contracts/handoffs.md`: subject, view, supported visible cues, alt/caption and provenance. Carpeta owns the fixed pencil-style prompt, provider call, timeout, crop/background removal, transparent notebook overlay and registration. The planner never writes a long provider prompt.

When the same concept benefits from physical recognition and an exact relationship diagram, both may be used: the illustration supplies recognition; the diagram carries the academic structure.
...
technical schematics. For each selected derived diagram:
...
4. materialize the whole plan with `scripts/visual_plan_hybrid.py`, which validates the
   decisions and delegates diagram entries to the existing collision-safe sketch generator;
...
diagram `reinterpret` is absent from the final Markdown, when its source asset
is used instead, or when the referenced derived record was not produced by the
deterministic SVG generator. `preserve+derived_sketch` must reference both
registered assets.

## Generated illustration materialization
A selected `visual_medium: illustration` is materialized by `scripts/visual_plan_hybrid.py` from the compact semantic spec in `02-plan.json`.

The bounded path is:
1. construct the provider prompt from the semantic spec plus Carpeta's fixed notebook style;
2. make one provider call;
3. deterministically crop the white working canvas and key it to transparency;
4. wrap the raster as a transparent notebook overlay and collision-safely register it under `assets/figures/`;
5. record provider/model identity, semantic-spec hash, prompt hash, deterministic seed, final asset hash and postprocessing metadata without storing credentials;
6. use only the exact asset returned by `02-visual-build.json`.

No independent per-image vision-review loop belongs to this path. The speed boundary is safe because generated illustrations are not allowed to carry authoritative academic structure. Normal academic review, integrity and final browser audit still apply to the document.

If the illustration provider is unavailable, do not retry repeatedly. Make at most one plan fallback for that optional visual: switch it to a deterministic diagram when the same supported meaning is naturally diagrammable, otherwise mark the optional illustration `visual_not_needed` for the current run. Never reduce academic text quality to preserve a visual.

The integrity gate fails closed when a planned illustration is absent, silently replaced by a source asset, or not backed by a registered `kind: illustration` record with `illustration_generation.method: generated-illustration`.
...
Do not describe every pixel. Explain the conceptual relationship the student should extract. Exact annotation for generated illustrations belongs outside the pixels.
...
Generated illustration pixels are never evidence for a factual claim. Never invent internal parts merely to make a drawing richer. If canonical evidence supports only a high-level object, request only a high-level recognizable form.
...
When creating a new diagram, save the asset under `assets/figures/` and register it with the established collision-safe deterministic generator. Derived ids are automatically namespaced `derived:` and registration refuses id/asset collisions. Never directly overwrite a record whose origin is `source`.
...
Deterministic diagram records retain their existing `generation.method: deterministic-svg`, generator identity/version and canonical spec hashes. Generated illustration records use a separate `illustration_generation.method: generated-illustration` object with generator/style version, semantic-spec hash, provider/model, seed, prompt hash and postprocessing identity; the figure record retains the final asset SHA-256. Secrets are never persisted. The metadata families stay distinct so a generated illustration cannot masquerade as an exact deterministic diagram.
