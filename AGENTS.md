...
2. Explicit project skills under `.agents/skills/` route to the corresponding file in `pipelines/`.
3. `rules/`, `pipelines/`, `contracts/` and `vendor/` are the canonical source of truth shared with Claude Code.
4. Codex-specific agent/parallel capabilities are optional optimizations described in `providers/codex.md`; correctness must not depend on them.
