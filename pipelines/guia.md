...
2. Start a run: `python scripts/venv_exec.py scripts/pipeline_run.py start --course <course-folder> --pipeline guia --scope "<scope>"`.
...
10. **INTEGRITY GATE** → run `study_validate_artifact` when MCP is connected or `python scripts/venv_exec.py scripts/artifact_integrity.py --type guide` otherwise; persist `<run-dir>/10-integrity.json`. Do not publish unless `ok: true`.
...
12. After both gates pass, copy accepted Markdown to `resumenes/_source/<scope>-guia.md`, publish `09-rendered.html` to `resumenes/<scope>-guia.html`, and mark the artifact with MCP or `python scripts/venv_exec.py scripts/artifact_state.py`.
