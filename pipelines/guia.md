...
12. **ATOMIC PUBLISH** → run `python scripts/venv_exec.py scripts/publish_artifact.py --markdown <accepted-md> --html <run-dir>/09-rendered.html --dest-markdown <course>/resumenes/_source/<scope>-guia.md --dest-html <course>/resumenes/<scope>-guia.html --report <run-dir>/11-publication.json`. Require byte-for-byte SHA-256 equality; do not use generic copy/edit operations for publication.
