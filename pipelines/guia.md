...
10. Publish `09-rendered.html` to `resumenes/<scope>-guia.html` and mark the HTML with `artifact_state.py mark --type guide --scope "<scope>"`. The HTML is the normal reading artifact; Markdown remains the portable source.
...
Before publishing any `guide` artifact, register newly derived figures with `study.py figures register-derived`, render with `render_study.py --check`, then run `scripts/artifact_integrity.py --type guide` and write `<run-dir>/10-integrity.json`. Publish only when it returns `ok: true`. Temporary helper scripts are allowed only under `<run-dir>/scratch/`, never in the course tree. Use stable `unit_id` from `01-input.json` for unit scope.
