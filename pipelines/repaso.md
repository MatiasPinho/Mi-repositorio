...
A normal `/repaso` run assumes the complete environment installed by `INSTALAR-STUDY.bat` (or `python -m pip install -r requirements.txt` plus `python -m playwright install chromium`). Check readiness with `python scripts/setup_env.py check`.
