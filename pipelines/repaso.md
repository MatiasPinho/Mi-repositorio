...
12. **ATOMIC PUBLISH** → resolve `<publish-dir>` as `unidades/<unit-id>/resumenes`, then run `python scripts/venv_exec.py scripts/publish_artifact.py --markdown <accepted-md> --html <run-dir>/09-rendered.html --dest-markdown <publish-dir>/_source/<scope>-repaso.md --dest-html <publish-dir>/<scope>-repaso.html --report <run-dir>/11-publication.json`. Require byte-for-byte SHA-256 equality; do not use generic copy/edit operations for publication.
