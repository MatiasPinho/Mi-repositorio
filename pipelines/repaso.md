...
10. Publish `09-rendered.html` to `resumenes/<scope>-repaso.html` and mark the HTML with `study_mark_artifact` when MCP is connected or `artifact_state.py mark --type rapid-review --scope "<scope>"` otherwise. The HTML is the normal reading artifact; Markdown remains the portable source.
...
Before publishing any `rapid-review` artifact, register newly derived figures with `study_register_derived_figure` when MCP is connected (otherwise `study.py figures register-derived`), render with `render_study.py --check`, then run `study_validate_artifact` when MCP is connected (otherwise `scripts/artifact_integrity.py --type rapid-review`) and persist `<run-dir>/10-integrity.json`. Publish only when it returns `ok: true`. Temporary helper scripts are allowed only under `<run-dir>/scratch/`, never in the course tree. Use stable `unit_id` from `01-input.json` for unit scope.
