...
12. **ATOMIC PUBLISH** → resolve `<publish-dir>` as the course `resumenes` directory. Only after both gates pass, run `python scripts/venv_exec.py scripts/publish_artifact.py --markdown <accepted-md> --html <run-dir>/09-rendered.html --dest-markdown <publish-dir>/_source/<scope>-resumen.md --dest-html <publish-dir>/<scope>-resumen.html --report <run-dir>/11-publication.json`. Require `ok: true` and matching SHA-256 values for source and destination. Do not publish with generic copy/edit operations.
