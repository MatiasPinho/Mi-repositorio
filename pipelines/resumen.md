...
3. **PLAN** → write `02-plan.json`, including `depth`. Use canonical knowledge to decide ordering, examples, traps and omissions. For every major concept decide `visual_required`, `visual_helpful` or `visual_not_needed`; for every selected visual record treatment, reason and provenance. `source-first` selects evidence, not final pixels. `preserve` / `preserve+derived_sketch` require a concrete `fidelity_reason`.
   ...
   Registered V2 scene ids are immutable revisions. A scene from an older visual policy is historical reference, not an automatically acceptable current composition. If the policy-composition guard reports that an identical registered scene has no PASS under the active policy, reconsider the concept from canonical knowledge and create a **new scene id and matching `derived_figure_id`**. If a failed review requires changing an already-registered scene's semantics/geometry, likewise create a new revision; never overwrite the registered old id.
...
4. **VISUAL BUILD** → complete visual work **before drafting**.
   ...
   **V2 default for new derived figures:**
   - before reuse or preview, run `python scripts/venv_exec.py scripts/visual_scene_policy_guard.py --course <course> --unit "<scope>" --plan <run-dir>/02-plan.json --write <run-dir>/02-visual-policy-guard.json`;
   - `02-visual-policy-guard.json -> ok: false` is a hard pre-preview design failure. Redesign only the cited policy-stale registered scenes from canonical knowledge under the current visual rules, use new append-only scene ids, update `02-plan.json`, and rerun the guard. Do **not** send a stale byte-identical composition to vision merely to see whether the new rubric approves it;
   - only after the policy guard passes may cross-run V2 PASS reuse be attempted according to the runtime optimization contract;
   ...
   - for tangible/recognizable concepts, `generic-box-substitution` is blocking under the visual rubric: a CPU/RAM/device represented primarily as a generic labeled rectangle must fail when canonical evidence safely supports a recognizable schematic cue. Repair by improving the drawing, not by adding prose inside the same box;
