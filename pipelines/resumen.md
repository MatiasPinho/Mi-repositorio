...
11. Only after the integrity gate passes, copy the accepted Markdown to `resumenes/_source/<scope>-resumen.md`, publish `09-rendered.html` to `resumenes/<scope>-resumen.html`, and mark the HTML with `study_mark_artifact` when MCP is connected or `artifact_state.py mark --type summary --scope "<scope>"` otherwise.
