...
class AtomicPublicationTests(unittest.TestCase):
    def test_publish_pair_is_identity_when_no_relocation_is_needed(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as td:
            ...
            self.assertEqual(dest_md.read_bytes(), before_md)
            ...
            for row in saved["files"]:
                self.assertEqual(row["source_sha256"], row["published_sha256"])
                self.assertEqual(row["published_sha256"], row["destination_sha256"])
    ...
    def test_republish_replaces_existing_files_without_truncation_or_temp_leaks(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as td:
            ...
            self.assertEqual(source_md.read_bytes(), expected_md)
            self.assertEqual(source_html.read_bytes(), expected_html)
    ...
    def test_publish_rebases_run_relative_images_without_mutating_validated_sources(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as td:
            ...
            self.assertEqual(source_md.read_bytes(), before_md)
            self.assertEqual(source_html.read_bytes(), before_html)
            self.assertIn('../../../assets/figures/u3-diagrama.png', source_md.read_text(encoding="utf-8"))
            self.assertIn('../../../assets/figures/u3-diagrama.png', source_html.read_text(encoding="utf-8"))
            ...
            self.assertNotEqual(by_role["html"]["source_sha256"], by_role["html"]["published_sha256"])
    ...
    def test_missing_local_resource_aborts_without_overwriting_existing_publication(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as td:
            ...
            original_source_md = source_md.read_bytes()
            original_source_html = source_html.read_bytes()
            ...
            self.assertEqual(source_md.read_bytes(), original_source_md)
            self.assertEqual(source_html.read_bytes(), original_source_html)
