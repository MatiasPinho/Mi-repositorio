...
def horizontal_flow_spec(count: int = 5) -> dict:
    ...
    return {
        ...
        "alt": "Pasos conectados horizontalmente.",
...
class SketchGeometryTests(unittest.TestCase):
    ...
    def test_three_step_horizontal_flow_fits_notebook_without_shrinking_fonts(self):
        report = sketch_geometry.analyze_spec(horizontal_flow_spec(3))
        self.assertTrue(report["ok"], report["issues"])
    ...
    def test_simple_vertical_flow_auto_reflows_horizontal_when_it_fits(self):
        spec = horizontal_flow_spec(3)
        spec["layout"]["direction"] = "top-to-bottom"
        report = sketch_geometry.analyze_spec(spec)
        self.assertTrue(report["ok"], report["issues"])
        self.assertEqual(report["effective_layout"]["direction"], "left-to-right")
        self.assertEqual(report["auto_direction"], sketch_geometry.AUTO_FLOW_POLICY)
    ...
    def test_connector_ports_land_on_node_borders_and_avoid_unrelated_nodes(self):
        ...
        self.assertEqual(len(report["connector_paths"]), 2)
    ...
    def test_installed_render_policy_embeds_notebook_typography_and_exact_edge_ports(self):
        ...
        self.assertRegex(edge_path, r"30\.00 50\.00$")
        ...
        self.assertIn(f'data-pencil-policy="{sketch_geometry.PENCIL_POLICY}"', text)
        ...
        self.assertEqual(render_report["geometry_policy"], sketch_geometry.GEOMETRY_POLICY)
